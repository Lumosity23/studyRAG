"""
Document embedding generation for vector search using SentenceTransformers.
"""
...
try:
    from ..utils.providers import get_embedding_model, get_embedding_model_name
except ImportError:
...
class EmbeddingGenerator:
    ...
    def __init__(
        ...
        logger.info(f"Embedding model loaded: {model_name}, dimensions: {self.dimensions}")
        
        self.config = {
            "dimensions": self.dimensions,
            "max_tokens": 512  # Most sentence transformers work well with this
        }
    ...
    async def generate_embeddings_batch(
        ...
        for text in texts:
            if not text or not text.strip():
                processed_texts.append("empty")  # SentenceTransformers needs non-empty text
                continue
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                embeddings = await loop.run_in_executor(
                    None,
                    lambda: self.model.encode(processed_texts, convert_to_numpy=True, batch_size=self.batch_size)
                )
...
def create_embedder(
    ...
    **kwargs
    ...
    """
    embedder = EmbeddingGenerator(model_name=model_name, **kwargs)
